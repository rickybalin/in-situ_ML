...
git clone https://github.com/al-rigazzi/SmartSim.git 
cd SmartSim
git checkout bb6b1a9
pip install -e .[ml] # The [ml] extension installs TensorFlow 2.8
cd ..

## Copy Redis and KeyDB config files
cp redis6.conf $ENV_PREFIX/SmartSim/smartsim/_core/config/
cp keydb.conf $ENV_PREFIX/SmartSim/smartsim/_core/config/
...
cd SmartRedis
git checkout 256dd7f873e689ffa08da8d326e90dfc02b7988b
pip install -e .
make lib
cd ..
...
mkdir lib
pip install fypp
cmake -B build -DCMAKE_INSTALL_PREFIX=$PWD/lib
cmake --build build
cmake --install build
cd ..
