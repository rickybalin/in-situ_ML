      ...
      integer nnDB, ppn, logging, niter
      real*8  t_init
...
c ==================================================
      ...
      allocate(t_data(niter-2,3))
      if (myrank.eq.0) then
        allocate(t_init_root(nproc))
        allocate(t_data_root((niter-2)*nproc,3))
      endif
...
c     Gather data to root rank
     ...
     &                   0, MPI_COMM_WORLD, ierr)
         call MPI_Gather(t_data(:,1), (niter-2), MPI_DOUBLE_PRECISION,
     &                   t_data_root(:,1), (niter-2), 
     &                   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
         call MPI_Gather(t_data(:,2), (niter-2), MPI_DOUBLE_PRECISION,
     &                   t_data_root(:,2), (niter-2), 
     &                   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
         call MPI_Gather(t_data(:,3), (niter-2), MPI_DOUBLE_PRECISION,
     &                   t_data_root(:,3), (niter-2), 
     &                   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
      else
...
c     Write data from root rank to file
      if (myrank.eq.0) then
         ...
         do n=1,nproc
            ...
            do i=1,niter-2
               write(11,*) t_data_root((n-1)*(niter-2)+i,1), 
     &                     t_data_root((n-1)*(niter-2)+i,2),
     &                     t_data_root((n-1)*(niter-2)+i,3)
            enddo
         enddo
         close(10)
         close(11)
...
c ==================================================
      ...
      real*4 x
      real*8 tic, toc
      integer samples, channels, pixels
      integer seed, num_inputs, offset, nGPU, batch
...
c     Start a do loop and perform send and receive data transfers
      do its=1,niter
         ...
         call sleep(2)
         toc = MPI_Wtime()
!d         write(*,*) "Slept for ",toc-tic," seconds"
...
!d         if (myrank.eq.0) write(*,*) 'Finished sending inference data'
         if (its.gt.2) t_data(its-2,1) = toc - tic
...
!         err = client%run_model('model', inputs, outputs)
     ...
     &      stop "ERROR: client%run_model failed"
         if (its.gt.2) t_data(its-2,2) = toc - tic
...
!d         if (myrank.eq.0) write(*,*) 'Finished retreiving predictions'
         if (its.gt.2) t_data(its-2,3) = toc - tic
